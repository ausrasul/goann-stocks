...
$id = `mysql -uroot -pd1sc0b4ll aktier -N -B -e 'select id from commodity where shortname = "$commodity"'`;
chomp($id);
...
open(PRICE, "mysql -uroot -pd1sc0b4ll aktier -N -B -e 'select dt,close,high,low from commodity_price where id=$id and dt>=\"$sdt\" and dt<=\"$edt\" order by dt asc'|");
while ($l = <PRICE>) {
	chomp($l);
	...
	if ($#close+1 > $maxpoints) {
		...
		print join(" ", @closepct) . "\n$n5lthree $n5ltwo $n5lone $n5one $n5two $n5three\n";
	}
}
close(PRICE);
